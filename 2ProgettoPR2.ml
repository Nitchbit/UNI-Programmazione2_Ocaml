...
type exp = (*data type*)
	...
	| Dictionary of (ide * exp) list (*il dizionario è una lista di coppie*)
	| Insert of ide * exp * exp (*key, value, dictionary*)
	| Delete of ide * exp (*key*)
	| Has_Key of ide * exp (*key, dictionary*)
	| Iterate of exp * exp (*fun, dictionary*)
	| Fold of exp * exp * exp (*fun, dictionary, default value*)
...
(*Aux Fun*)
let rec memberDict d x = match d with
	| [] -> false
	| (z,y)::xs -> if (x=z) then true else memberDict xs x;;

let rec withoutDup d = match d with
	| [] -> []
	| (x,y)::xs -> if (memberDict xs x) then withoutDup xs else (x,y)::withoutDup xs;;

let rec memberList l x = match l with
	| [] -> false
	| y::xs -> if (x=y) then true else memberList xs x;;

let rec typec d s = match d with
	| [] -> []
...
let rec eval (e : exp) (r : evT env) : evT = match e with
	...
	| Dictionary(lis) -> let thisd = withoutDup lis
		in DictionaryVal(evalD thisd r)
	| Insert(i, v, d) -> (match (eval d r) with
		| DictionaryVal(myd) -> if (memberDict myd i) then failwith("Key already inserted")
								else (match myd with
	...
	| Delete(i, d) -> (match (eval d r) with
		| DictionaryVal(myd) -> if (memberDict myd i) then DictionaryVal(remEl myd i)
								else failwith("Key Not Found")
		| _ -> failwith("Wrong Value"))
	| Has_Key(i, d) -> (match (eval d r) with
		| DictionaryVal(myd) -> Bool(memberDict myd i)
		| _ -> failwith("Wrong Valure"))
	| Iterate(f, d) -> (match (eval d r) with
		| DictionaryVal(myd) -> DictionaryVal(appIt myd f r)
		| _ -> failwith("Wrong Value"))
	| Fold(f, d, v) -> (match (eval d r) with
		| DictionaryVal(myd) -> appFold myd f r v
		| _ -> failwith("Wrong Value"))
	| Filter(l, d) -> (match (eval d r) with
		| DictionaryVal(myd) -> DictionaryVal(appFil myd l r)
		| _ -> failwith("Wrong Value"))
	and remEl (d : (ide * evT) list) (i : ide) = (match d with
		| [] -> []
		| (x,y)::xs -> if (i=x) then (remEl xs i) else (x,y)::(remEl xs i))
	and evalD (d : (ide * exp) list) (r : evT env) = let ed = (match d with
		| [] -> []
		| (x,y)::xs -> (x, eval y r)::(evalD xs r))
		in checkt ed
	and checkt (d : (ide * evT) list) = (match d with
		| [] -> []
	...
	and appIt (d : (ide * evT) list) (f : exp) (r : evT env) =
		(match d with
			| [] -> []
			| (x,y)::xs -> (x, (dFCall f y r))::appIt xs f r)
	and appFold (d : (ide * evT) list) (f : exp) (r : evT env) (n : exp) = match f with
		| FunBin(i1, i2, a) -> (match d with
			| [] -> eval n r
			| (x,y)::xs -> evalBinFun a r i1 i2 y (appFold xs f r n))
		| Fun(_,_) -> failwith("Not Binary")
		| _ -> failwith("Wrong Value")
	and evalBinFun (a : exp) (amb : evT env) (i1 : ide) (i2 : ide) (y : evT) (n : evT) =
		eval a (bind (bind amb i2 n) i1 y)
	and appFil (d : (ide * evT) list) (l : ide list) (r : evT env) =
		(match d with
			| [] -> []
			| (x,y)::xs -> if memberList l x then (x,y)::appFil xs l r else appFil xs l r)
